...
pub extern "C" fn rust_echo_c(ptr: *const c_char) -> *mut c_char {
    ...
    match c_string.to_bytes() {
        bytes => utf8_cstring(bytes)
            .map(CString::into_raw)
            .unwrap_or_else(ptr::null_mut),
    }
}
...
mod tests {
    ...
    fn roundtrip_from_bytes() {
        ...
        let pointer = rust_echo(payload.as_ptr(), payload.len());
    ...
    fn roundtrip_from_c_string() {
        ...
        let pointer = rust_echo_c(original.as_ptr());
        assert!(!pointer.is_null());
    ...
    fn invalid_utf8_returns_null() {
        ...
        let pointer = rust_echo(bytes.as_ptr(), bytes.len());
        assert!(pointer.is_null());
    }

    #[test]
    fn null_pointer_with_zero_length_returns_empty_string() {
        let pointer = rust_echo(ptr::null(), 0);
        assert!(!pointer.is_null());
